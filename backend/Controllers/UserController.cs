...
public UserController(DataContext context, Authenticator passwordHasher)
{
  ...
  public async Task<ActionResult<object>> GetNameById(int id)
  {
    var user = await _context.Users.FindAsync(id);
    if (user is null)
      return NotFound("User not found.");
  ...
  [HttpGet("GetStudentIds")]
  public async Task<ActionResult<List<int>>> GetStudentIds()
  {
    var userIds = await _context.Users.Where(e => e.Role == Role.Student).Select(e => e.Id).ToListAsync();
    if (userIds is null)
